...
import { BlockOutlined, CancelOutlined, DisabledByDefaultOutlined, DoDisturbAltOutlined, EventBusyOutlined } from "@mui/icons-material";
import Filler from "app/sections/Filler";

const HostingBookingsGroupedCanceledBookings = ({
    ...
    useEffect(() => {
        if (reservations?.length) {
            setFiltered(reservations.filter((res) => res.reservationInfo?.status === ReservationStatus.Canceled))
        }
    }, [reservations])
                    ...
                    {loading? (
                        ...
                        <>
                            {filtered?.length? (
                                ...
                                <Filler
                                    LargeIcon={BlockOutlined}
                                    title={'No canceled reservations'}
                                    desc={'You have no canceled reservations.'}
                                    sx={{
                                        px:0,
                                        width:'100%',
                                    }}
...
export default HostingBookingsGroupedCanceledBookings;