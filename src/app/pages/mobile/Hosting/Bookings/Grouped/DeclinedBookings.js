...
const HostingBookingsGroupedDeclinedBookings = ({
    ...
    useEffect(() => {
        if (reservations?.length) {
            setFiltered(reservations.filter((res) => res.reservationInfo?.status === ReservationStatus.Declined))
        }
    }, [reservations])
                    ...
                    {loading? (
                        ...
                        <>
                            {filtered?.length? (
                                ...
                                <Filler
                                    LargeIcon={EventBusyOutlined}
                                    title={'No declined reservations'}
                                    desc={'You have no declined reservations.'}
                                    sx={{
                                        width:'100%',
                                    }}
                                />
...
export default HostingBookingsGroupedDeclinedBookings;